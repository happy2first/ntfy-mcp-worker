...
export function parsePriority(value: unknown): number | undefined {
  ...
  // Modern browsers/HTTP stacks may automatically send RFC 9218's HTTP
  // Priority header (for example `Priority: u=1, i`). ntfy historically also
  // accepts a message header named `Priority`, so the two namespaces collide.
  // Ignore structured HTTP priority values while preserving ntfy values such
  // as `high`, `urgent`, or `4`.
  const httpPriorityTokens = text.split(",").map((token) => token.trim()).filter(Boolean);
  if (httpPriorityTokens.length && httpPriorityTokens.every((token) => token === "i" || /^u=[0-7]$/.test(token))) return undefined;
