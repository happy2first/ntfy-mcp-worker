...
import { normalizeBaseUrl, parseSince, randomId, sha256Hex } from "./protocol.js";
...
type MessageRow = {
  ...
  cache_visible: number;
  delivered: number;
  scheduled_at: number | null;
  admin_delete_at: number | null;
  base_url: string;
  created_at: string;
};
...
function publicAttachment(attachment?: StoredAttachment): StoredAttachment | undefined {
  if (!attachment) return undefined;
  const { key: _key, ...rest } = attachment;
  return rest;
}
...
export class NtfyServerDO extends DurableObject<Env> {
  ...
  private ensureSchema() {
    this.ctx.storage.sql.exec(`CREATE TABLE IF NOT EXISTS messages (
      ...
      content_type TEXT,
      encoding TEXT,
      cache_visible INTEGER NOT NULL DEFAULT 1,
      delivered INTEGER NOT NULL DEFAULT 1,
      scheduled_at INTEGER,
      admin_delete_at INTEGER,
    ...
    this.ctx.storage.sql.exec(`CREATE TABLE IF NOT EXISTS settings (
  ...
  private insert(message: StoredMessage, baseUrl: string) {
    ...
    this.ctx.storage.sql.exec(
      ...
      message.tags?.length ? JSON.stringify(message.tags) : null,
      message.click || null,
      message.icon || null,
      message.actions?.length ? JSON.stringify(message.actions) : null,
      ...
      message.cache_visible ? 1 : 0,
      message.delivered ? 1 : 0,
      message.scheduled_at || null,
      message.admin_delete_at || null,
  ...
  private formatChunk(format: StreamSubscriber["format"], message: NtfyMessage): Uint8Array | null {
    if (format === "raw") {
      ...
      return encoder.encode(`${message.message || ""}\n`);
    }
  ...
  private publishToLocalSubscribers(message: StoredMessage) {
    ...
    for (const [id, subscriber] of this.streamSubscribers) {
      ...
      catch { this.streamSubscribers.delete(id); }
    }
    for (const ws of this.ctx.getWebSockets()) {
      try {
        ...
        try { ws.close(1011, "delivery failed"); } catch { /* ignored */ }
      }
    }
  }
  ...
  private async forwardPollRequest(message: StoredMessage, baseUrl: string) {
    ...
    const topicUrl = `${exactBaseUrl}/${message.topic}`;
    const topicHash = await sha256Hex(topicUrl);
    const headers = new Headers({ "X-Poll-ID": message.id, "User-Agent": "ntfy-mcp-worker/0.1" });
  ...
  private async deletePhysical(ids: string[]) {
    ...
    const rows = this.ctx.storage.sql.exec<{ attachment_json: string | null }>(
      `SELECT attachment_json FROM messages WHERE id IN (${placeholders})`, ...unique,
    ).toArray();
    const keys = rows.map((row) => parseJson<StoredAttachment | null>(row.attachment_json, null)?.key).filter((v): v is string => Boolean(v));
    this.ctx.storage.sql.exec(`DELETE FROM messages WHERE id IN (${placeholders})`, ...unique);
    if (keys.length) await Promise.all(keys.map((key) => this.env.ATTACHMENTS.delete(key).catch(() => undefined)));
    return { deleted: unique.length, attachmentObjectsDeleted: keys.length };
  }

  private async scheduleNextAlarm() {
    this.ensureSchema();
    const now = nowSeconds();
    const candidates = this.ctx.storage.sql.exec<{ due: number | null }>(`
    ...
    `).toArray()[0];
    const due = Number(candidates?.due || 0);
    if (!due) {
  ...
  async alarm() {
    ...
    for (const row of due) {
      this.ctx.storage.sql.exec("UPDATE messages SET delivered=1 WHERE id=?", row.id);
      const message = rowToStored({ ...row, delivered: 1 });
      await this.deliver(message, row.base_url);
    }
  ...
  async webSocketMessage(_ws: WebSocket, _message: string | ArrayBuffer) {
    // ntfy WebSocket subscriptions are server-to-client; inbound frames are ignored.
  }

  async webSocketClose(ws: WebSocket, code: number, reason: string, wasClean: boolean) {
    try { ws.close(code, reason); } catch { /* already closed */ }
    console.debug("ntfy websocket closed", { code, reason, wasClean });
  }
  ...
  private async handlePublish(request: Request) {
    ...
    if (!message.expires) message.expires = message.cache_visible ? message.time + retention : Math.max(message.time, nowSeconds()) + 300;
  ...
  private handleSubscribe(request: Request) {
    ...
    if (poll) {
      if (format === "json" && url.searchParams.get("id")) {
        return rows[0] ? json(toPublic(rows[0])) : json({ code: 40401, http: 404, error: "message not found" }, 404);
      }
  ...
  private handleTopics() {
    this.ensureSchema();
    const rows = this.ctx.storage.sql.exec<{ topic: string; messages: number; last_time: number }>(
  ...
  private handleStatus() {
    ...
    const counts = this.ctx.storage.sql.exec<{ messages: number; topics: number; scheduled: number; pending_deletes: number }>(`
      SELECT COUNT(*) AS messages, COUNT(DISTINCT topic) AS topics,
        ...
        SUM(CASE WHEN admin_delete_at IS NOT NULL THEN 1 ELSE 0 END) AS pending_deletes
      FROM messages
    `).toArray()[0];
    return json({
      ok: true,
      ...
      pendingDeletes: Number(counts?.pending_deletes || 0),
      websocketSubscribers: this.ctx.getWebSockets().length,
      streamSubscribers: this.streamSubscribers.size,
      databaseBytes: Number(this.ctx.storage.sql.databaseSize || 0),
      retentionSeconds: this.retentionSeconds(),
      upstreamBaseUrl: this.env.UPSTREAM_BASE_URL || "https://ntfy.sh",
      baseUrl: this.env.BASE_URL || null,
    });
  }
  ...
  private async handleSetRetention(request: Request) {
    ...
    this.ctx.storage.sql.exec("INSERT OR REPLACE INTO settings(key,value,updated_at) VALUES('message_retention_seconds',?,?)", String(seconds), now);
  ...
  private async handleManage(request: Request) {
    ...
    if (event === "message_delete") {
      const scheduled = this.ctx.storage.sql.exec<{ id: string }>(
        "SELECT id FROM messages WHERE topic=? AND sequence_id=? AND delivered=0", topic, sequenceId,
      ).toArray().map((row) => row.id);
      await this.deletePhysical(scheduled);
    }
    const now = nowSeconds();
    const message: StoredMessage = {
      id: randomId(), sequence_id: sequenceId, time: now, expires: now + this.retentionSeconds(), event, topic,
      cache_visible: true, delivered: true, created_at: new Date().toISOString(),
    };
    this.insert(message, baseUrl);
    await this.deliver(message, baseUrl);
    await this.scheduleNextAlarm();
    return json(toPublic(message));
  }
  ...
  private async handleAdminDelete(request: Request) {
    ...
    const result = await this.deletePhysical(ids);
    await this.scheduleNextAlarm();
  ...
  private async handleScheduleDelete(request: Request) {
    ...
    if (body.deleteAt == null || body.deleteAt === "") {
      this.ctx.storage.sql.exec("UPDATE messages SET admin_delete_at=NULL WHERE id=?", id);
      await this.scheduleNextAlarm();
    ...
    this.ctx.storage.sql.exec("UPDATE messages SET admin_delete_at=? WHERE id=?", deleteAt, id);
    await this.scheduleNextAlarm();
