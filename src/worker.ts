...
export { NtfyServerDO } from "./ntfy-do.js";
...
function serverStub(env: Env) { return env.NTFY_SERVER.get(env.NTFY_SERVER.idFromName(SERVER_DO_NAME)); }
async function callServer(env: Env, path: string, init?: RequestInit): Promise<Response> { return serverStub(env).fetch(`https://ntfy.internal${path}`, init as any); }
...
function topicAuthConfigured(env: Env) { return Boolean(env.NTFY_ACCESS_TOKEN || (env.NTFY_USERNAME && env.NTFY_PASSWORD)); }
...
function protocolDenied() { return Response.json({ code: 40101, http: 401, error: "unauthorized" }, { status: 401, headers: { "www-authenticate": "Basic realm=\"ntfy\", Bearer" } }); }
function adminDenied(env: Env) {
...
function baseUrl(request: Request, env: Env) { return normalizeBaseUrl(env.BASE_URL || new URL(request.url).origin); }
...
function messageDefaults(topic: string, env: Env, scheduledAt?: number): StoredMessage {
  ...
  return {
    id: randomId(), time, event: "message", topic, message: "", cache_visible: true,
    delivered: !scheduledAt, ...(scheduledAt ? { scheduled_at: scheduledAt } : {}), created_at: new Date().toISOString(),
  };
}
...
async function uploadAttachmentToDo(request: Request, env: Env, msg: StoredMessage, filename: string, publicBaseUrl: string): Promise<StoredAttachment> {
  ...
  const headers = new Headers({ "content-type": type, "x-filename": safeName });
  if (length) headers.set("content-length", String(length));
  const response = await callServer(env, `/attachment/${encodeURIComponent(msg.id)}`, { method: "POST", headers, body: request.body });
  const data = await response.json().catch(() => ({})) as any;
  if (!response.ok) throw new Error(data.error || data.message || `attachment_upload_failed_${response.status}`);
  return {
    name: safeName, type, size: Number(data.size || length || 0) || undefined,
    url: `${publicBaseUrl}/file/${msg.id}/${encodeURIComponent(safeName)}`, ref: String(data.attachmentRef || "") || undefined,
  };
}

async function cleanupUploadedAttachment(env: Env, messageId: string) {
  try { await callServer(env, `/attachment/${encodeURIComponent(messageId)}`, { method: "DELETE" }); } catch {}
}
...
async function parseHttpPublish(request: Request, env: Env, topic: string, sequenceId?: string): Promise<StoredMessage> {
  ...
  if (boolHeaderOrQuery(request, url, ["X-Markdown", "Markdown", "MD"], false) || contentType === "text/markdown") msg.content_type = "text/markdown";
  ...
  if (externalAttach) {
    msg.attachment = { name: sanitizeFilename(filename || externalAttach.split("/").pop() || "attachment"), url: externalAttach };
    ...
    msg.attachment = await uploadAttachmentToDo(request, env, msg, filename || "attachment.bin", baseUrl(request, env));
    msg.message = explicitMessage || filename || "Attachment";
  } else msg.message = explicitMessage || await request.text();
...
async function publishStored(env: Env, msg: StoredMessage, publicBaseUrl: string): Promise<NtfyMessage> {
  try {
    return await callServerJson<NtfyMessage>(env, "/publish", {
      method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ message: msg, baseUrl: publicBaseUrl } satisfies PublishInput),
    });
  } catch (error) {
    if (msg.attachment?.ref) await cleanupUploadedAttachment(env, msg.id);
    throw error;
  }
}
...
async function manageMessage(env: Env, topic: string, sequenceId: string, event: "message_delete" | "message_clear", publicBaseUrl: string) {
  return callServerJson<NtfyMessage>(env, "/manage", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ topic, sequenceId, event, baseUrl: publicBaseUrl }) });
}
...
async function handleAttachment(request: Request, env: Env, path: string[]): Promise<Response> {
  ...
  const upstream = await callServer(env, `/attachment/${encodeURIComponent(id)}?name=${encodeURIComponent(name)}`, { method: request.method });
...
async function handleProtocol(request: Request, env: Env): Promise<Response> {
  ...
  const url = new URL(request.url), route = parseNtfyRoute(url.pathname);
  try {
    if (route.kind === "file") return handleAttachment(request, env, route.segments);
    if (route.kind === "subscribe") {
      if (request.method !== "GET") return new Response("Method Not Allowed", { status: 405 });
      const params = new URLSearchParams(url.searchParams);
      params.set("topics", parseTopics(route.topics).join(",")); params.set("format", route.format);
      return callServer(env, `/subscribe?${params}`);
    }
    if (route.kind === "manage_alias") {
      const event = route.action === "delete" ? "message_delete" : "message_clear";
      return Response.json(await manageMessage(env, validateTopic(route.topic), validateSequenceId(route.sequenceId), event, baseUrl(request, env)));
    }
    if (route.kind === "topic_sequence" && request.method === "DELETE") return Response.json(await manageMessage(env, validateTopic(route.topic), validateSequenceId(route.sequenceId), "message_delete", baseUrl(request, env)));
...
function createServer(env: Env) {
  ...
  server.registerTool("ntfy_publish_message", {
    ...
    inputSchema: {
      topic: z.string().min(1).max(64).optional(), message: z.string().max(MAX_TEXT_BYTES), title: z.string().max(256).optional(),
      priority: z.union([z.number().int().min(1).max(5), z.enum(["min", "low", "default", "high", "max", "urgent"])]).optional(),
      tags: z.array(z.string().max(64)).max(20).optional(), click: z.string().url().optional(), icon: z.string().url().optional(), markdown: z.boolean().optional(),
      actions: z.array(z.any()).max(3).optional(), sequenceId: z.string().min(1).max(64).optional(), delay: z.string().optional(), cache: z.boolean().optional(),
    },
  }, async (args) => {
  ...
  server.registerTool("ntfy_fetch_messages", {
    ...
    inputSchema: { topics: z.array(z.string().min(1).max(64)).min(1).max(20).optional(), since: z.string().default("all"), limit: z.number().int().min(1).max(500).default(100), scheduled: z.boolean().default(false) },
  ...
  server.registerTool("ntfy_delete_record", {
    description: "管理操作：立即物理删除一条服务端消息记录及其本地附件。",
    inputSchema: { id: z.string().min(1).max(64) },
  }, async ({ id }) => result(await callServerJson(env, "/admin/delete", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ ids: [id] }) })));
  server.registerTool("ntfy_status", { description: "查看消息、topic、调度、订阅、DO SQLite 与附件分块存储占用，以及 iOS upstream 配置。", inputSchema: {} }, async () => result(await callServerJson(env, "/admin/status")));
  return server;
}
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (url.pathname === "/admin" || url.pathname.startsWith("/admin/api/") || url.pathname === "/mcp") {
      ...
      if (url.pathname.startsWith("/admin/api/")) {
        ...
        if (suffix === "/test-publish" && request.method === "POST") {
          ...
          const msg = normalizeJsonPublish({ topic, title: "ntfy-mcp-worker 测试", message: "来自 ntfy MCP Worker 管理页的测试通知", priority: "high", tags: ["white_check_mark"] }, topic, env);
          return Response.json(await publishStored(env, msg, baseUrl(request, env)));
        }
        const map: Record<string, string> = { "/status": "/admin/status", "/topics": "/admin/topics", "/messages": "/admin/messages", "/delete": "/admin/delete", "/storage-limit": "/admin/storage-limit" };
        const internal = map[suffix];
        if (!internal) return new Response("Not Found", { status: 404 });
        return proxyAdmin(request, env, `${internal}${request.method === "GET" ? url.search : ""}`);
      }
    ...
    if (url.pathname === "/") {
      ...
      return Response.json({
        ok: true, service: "ntfy-mcp-worker", version: VERSION, protocol: "ntfy-compatible", mcp: "/mcp", admin: "/admin", health: "/v1/health",
        iosUpstream: env.UPSTREAM_BASE_URL || "https://ntfy.sh", attachmentStorage: "durable-object-sqlite",
      });
    }
