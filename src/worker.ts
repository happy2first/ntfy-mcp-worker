...
function messageDefaults(topic: string, env: Env, scheduledAt?: number): StoredMessage {
  ...
  return {
    id: randomId(), time, expires: time + retentionSeconds(env), event: "message", topic, message: "", cache_visible: true,
...
async function uploadAttachmentToDo(request: Request, env: Env, msg: StoredMessage, filename: string, publicBaseUrl: string): Promise<StoredAttachment> {
  ...
  const headers = new Headers({ "content-type": type, "x-filename": safeName, "x-expires": String(expires) });
  ...
  return {
    name: safeName, type, size: Number(data.size || length || 0) || undefined, expires,
...
function createServer(env: Env) {
  ...
  server.registerTool("ntfy_delete_record", {
    description: "管理操作：物理删除服务端消息记录，或为单条记录设置/取消定时删除。若记录有本地附件，会在同一 DO SQLite 事务中一并删除。",
