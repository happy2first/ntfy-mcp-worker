...
import { capacityStatus, enforceCapacity, setCapacityLimit } from "./capacity-retention.js";
...
export class NtfyServerDO extends DurableObject<Env> {
  ...
  private async scheduleNextAlarm() {
    ...
    await this.ctx.storage.setAlarm(Math.max(Date.now() + 1000, due * 1000));
  }
  ...
  async alarm() {
    ...
    enforceCapacity(this.ctx.storage);
  ...
  private async handleAttachmentUpload(request: Request, messageId: string) {
    ...
    this.ctx.storage.transactionSync(() => {
      ...
      this.ctx.storage.sql.exec(
        ...
        attachmentRef, messageId, name, mimeType, bytes.byteLength, chunkCount, 0, createdAt,
      );
    ...
    return json({ attachmentRef, messageId, name, type: mimeType, size: bytes.byteLength });
  }
  ...
  private async handlePublish(request: Request) {
    ...
    message.admin_delete_at = undefined;
  ...
  private handleAdminMessages(url: URL) {
    ...
    return json({ total, limit, offset, messages: rows.map((row) => {
      ...
      return { ...toPublic(stored), delivered: stored.delivered, cacheVisible: stored.cache_visible, scheduledAt: stored.scheduled_at || null, createdAt: stored.created_at };
    }) });
  }
  ...
  private handleStatus() {
    const counts = this.ctx.storage.sql.exec<{ messages: number; topics: number; scheduled: number }>(`
      SELECT COUNT(*) AS messages,COUNT(DISTINCT topic) AS topics,
        SUM(CASE WHEN delivered=0 THEN 1 ELSE 0 END) AS scheduled FROM messages
    `).toArray()[0];
    const capacity = capacityStatus(this.ctx.storage);
    return json({
      ok: true,
      ...
      historyLimitBytes: capacity.state.limitBytes,
      historyTargetBytes: capacity.targetBytes,
      historyBytes: capacity.usage.historyBytes,
      messageBytes: capacity.usage.messageBytes,
      databaseBytes: capacity.usage.databaseBytes,
      attachmentCount: capacity.usage.attachmentCount,
      attachmentBytes: capacity.usage.attachmentBytes,
      attachmentChunks: capacity.usage.attachmentChunks,
      lastCleanupAt: capacity.state.lastCleanupAt || null,
      totalDeletedMessages: Number(capacity.state.totalDeletedMessages || 0),
      totalDeletedAttachmentBytes: Number(capacity.state.totalDeletedAttachmentBytes || 0),
      attachmentSingleFileLimitBytes: MAX_ATTACHMENT_BYTES,
      attachmentChunkBytes: ATTACHMENT_CHUNK_BYTES,
      upstreamBaseUrl: this.env.UPSTREAM_BASE_URL || "https://ntfy.sh",
      baseUrl: this.env.BASE_URL || null,
    });
  }

  private async handleSetStorageLimit(request: Request) {
    const body = await request.json<{ limitMB?: number }>();
    try {
      return json(setCapacityLimit(this.ctx.storage, body.limitMB));
    } catch (error) {
  ...
  private async handleManage(request: Request) {
    ...
    const now = nowSeconds();
    const message: StoredMessage = { id: randomId(), sequence_id: sequenceId, time: now, event, topic, cache_visible: true, delivered: true, created_at: new Date().toISOString() };
    this.insert(message, baseUrl); await this.deliver(message, baseUrl); enforceCapacity(this.ctx.storage); await this.scheduleNextAlarm();
    return json(toPublic(message));
  }
  ...
  async fetch(request: Request): Promise<Response> {
    try {
      ...
      if (url.pathname === "/admin/storage-limit" && request.method === "POST") return await this.handleSetStorageLimit(request);
